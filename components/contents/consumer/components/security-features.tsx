...
export const SecurityFeatureCard = ({
  ...
  return (
    <Card
      className={clsx(
        "overflow-hidden rounded-xl border-none shadow-md hover:shadow-xl",
        className
      )}
    >
...
export const AIFeatureCard = ({
  ...
  return (
    <Link href={buttonLink}>
      <Card
        className={clsx(
          "group overflow-hidden rounded-xl p-0 text-white hover:cursor-pointer hover:shadow-lg",
          className
        )}
      >
        <CardHeader className="pb-2">
        ...
        <CardContent className="space-y-4 pb-0">
          <Button variant="link" className="p-0 text-sm text-white group-hover:underline">
            {buttonText}
          </Button>
          {showImage && (
            <div className="relative w-[14.375rem]">
              <Image
                src="/assets/images/consumer/ai-chip.svg"
                alt="AI Chip"
                width={100}
                height={200}
                className="h-full w-full object-cover transition-all duration-500 group-hover:rotate-[-1.75deg] group-hover:scale-110"
              />
            </div>
          )}
        </CardContent>
...
export const SecurityFeatures = () => {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
      ...
      <AIFeatureCard title="AI-Enhanced protection" className="bg-primary" />
    </div>
  )
}
