      ...
      scrollCtx = gsap.context(() => {
        ...
        if (!reducedMotion) {
          tl.fromTo(
            ...
            { opacity: 0, y: -120, scale: 0.95, ease: "power1.inOut", duration: 0.6 },
            0
          )
            .fromTo(
              cornerRef.current,
            ...
            .fromTo(
              ...
              { scale: 0.75, borderRadius: "32px", filter: "brightness(0.4)", ease: "power2.inOut", duration: 1 },
              0
            )
        }
      }, heroRef)
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: "linear-gradient(to bottom, rgba(4,10,6,0.4) 0%, transparent 40%, rgba(4,10,6,0.8) 100%)" }}
        />
        <div
          aria-hidden="true"
          className="absolute inset-0"
          style={{ background: "radial-gradient(circle at 20% 80%, rgba(16,185,129,0.12) 0%, transparent 50%)" }}
        />
      </div>

      {/* Content — single headline + 2 CTAs only */}
      <div className="absolute inset-0 z-10 flex flex-col justify-end px-6 pb-16 md:px-16 md:pb-20">
        <div className="flex items-end justify-between gap-6">
          <div ref={contentRef} className="max-w-[700px]">
            <h1
              className="mb-10 font-semibold tracking-tight text-white drop-shadow-sm"
              style={{ fontSize: "clamp(44px, 6.5vw, 86px)", lineHeight: 1.08, letterSpacing: "-0.03em" }}
            >
              Buy safe.{" "}
              <span className="bg-gradient-to-r from-emerald-300 via-green-400 to-emerald-500 bg-clip-text text-transparent">
                Return bottles. Earn rewards.
              </span>
            </h1>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <a
                href="/download"
                className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-white px-7 py-3.5 text-[15px] font-bold text-black transition-all duration-300 hover:scale-105 hover:shadow-[0_0_30px_rgba(255,255,255,0.35)] active:scale-95"
              >
                ...
                <Download size={17} className="relative z-10 transition-transform group-hover:-translate-y-0.5" />
          ...
          {/* Scroll indicator */}
          <div ref={cornerRef} className="hidden shrink-0 flex-col items-end gap-3 text-right md:flex">
            <div className="flex h-10 w-6 justify-center rounded-full border border-white/20 bg-white/5 p-1 backdrop-blur-sm">
              <div className="h-2 w-1.5 animate-bounce rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]" />
            </div>
            <span className="text-[11px] font-bold uppercase tracking-widest text-white/35">Scroll</span>
          </div>
        </div>
      </div>
    </div>
